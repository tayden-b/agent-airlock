...
async function fetchSessions(apiKey: string): Promise<unknown[]> {
  const res = await fetch(`${API_BASE}/v3/sessions?limit=50`, {
    headers: { authorization: `Bearer ${apiKey}` },
  });
  if (!res.ok) {
    // Older orgs expose v1 with the same shape.
    const v1 = await fetch(`${API_BASE}/v1/sessions?limit=50`, {
      headers: { authorization: `Bearer ${apiKey}` },
    });
    if (!v1.ok) throw new Error(`devin api ${res.status}/${v1.status}`);
    return sessionsOf(await v1.json());
  }
...
function sessionsOf(body: unknown): unknown[] {
  ...
  const b = body as { sessions?: unknown[]; data?: unknown[] };
  return b.sessions ?? b.data ?? [];
}

export async function syncDevinSessions(): Promise<{ sessions: number; ingested: number }> {
  const apiKey = process.env.DEVIN_API_KEY;
  if (!apiKey) return { sessions: 0, ingested: 0 };
