...
- The "live" SSE indicator can stay "connecting" for up to ~15 s after page load: `/api/stream` only writes on heartbeat (`stream.heartbeatMs` = 15 s) or a real event, so `EventSource.onopen` may not fire until the first flush. Not a bug unless it never flips.
- After `SessionEnd` the run shows status `ended` but the `main` agent may still show `running` (subagents correctly show `completed`) — known inconsistency, verify before reporting as a new bug.
